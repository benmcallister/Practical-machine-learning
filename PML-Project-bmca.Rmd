...
Due to the realtively high out-of-sample error for the decision tree, I decided to try a random forest model. 
...
This model was very accurate, but took a very long time to run. I decided to try preprocessing the data with PCA in order to train a simpler, faster model.
...
Ultimately, because the out-of-sample error rate was higher on the PCA-based model, I decided to use the non-PCA-based model (my second model) for my final submission.